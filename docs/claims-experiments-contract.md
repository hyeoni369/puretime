...
|----|---------------------|-----------|-----------|------|------|-----------|
| C1 | noise-free makespan이 solo 기준(GT) 대비 오차 E% 이내 | 1-1, 1-2, 1-4 | core | **P0** | **설계** | 그룹1 단일-노이즈 **한 세트**에서 1-1·1-2·1-4 동시 추출. victim 4종, K=50, solo 분포 GT, 분포+오차막대. **E2E K=30/50(06-16): CPU 98%@1.0→3.1×(강도 sweep)·Net 89%@4.8×·Block 91%@3.2×(전제 2개: queue_depth=2 + 채워진 HDD + store-모드 I/O-bound victim, 과다제거 꼬리 ~13% 명시). 기본 depth=32에선 Block 39%(NCQ가 issue→complete에 숨김); 빈 디스크에선 45%(큐 안 쌓임) — 둘 다 전제 위반. fig1b robustness=CPU+Net(Block은 디스크-상태 의존이라 제외, 1a 91% 대표값만).** |
...
| C7 | 트레이싱 추가 지연·자원 비용이 낮음 | 4-1 | C6 | **P0** | **완료** | **메인=자원 fig4(06-18):** PureTime online 비용 = 노드 자원의 **<0.1%**(CPU 0.05~0.08% of 24코어 = 한 코어 1-2%; RSS 71MB = 노드 94GB RAM의 0.075%, ring buffer 32MB 지배·조절가능). online만(Analyzer offline 제외, critical path 밖). **시간 오버헤드를 메인으로 안 내세우는 이유: 별도 userspace 프로세스(loader)가 ring buffer를 비워 함수 critical path엔 가벼운 커널훅만 → 노이즈 이하(<1%).** **보조=이벤트율 곡선 fig3:** 오버헤드 ∝ switch/s(victim=`ctxsw-bench` 핑퐁, 경쟁노이즈 없이 sched_switch 결정적 생성), 12K→717K switch/s에서 +0.9%→+44% 단조-양수 — 시간 오버헤드가 *왜* 작은지 정량 뒷받침. (box plot=victim 절대시간 with/without 비교는 오버헤드<측정노이즈라 세션마다 부호가 바뀜 → 폐기.) |
