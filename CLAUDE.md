...
**Tracer (`src/puretime.bpf.c`)** — 9 active eBPF programs emitting binary structs to the `events` ring buffer via `bpf_ringbuf_reserve/submit` (no JSON in the kernel; a reserve failure bumps a per-CPU `dropped_events` counter). Hooks: `fentry/enqueue_task` + `tp_btf/sched_switch` (sched), `tp_btf/net_dev_{queue,start_xmit}`, `tp_btf/block_rq_{insert,issue,complete}`, `tp_btf/softirq_{entry,exit}`, plus `fentry/tcp_sendmsg`+`tcp_close` (socket→cgroup tracking). `block_rq_complete` is consumed by the analyzer's **device-queue wait** model (a request's `[issue,complete)` is its device occupancy; the victim waits when its lifetime overlaps a foreign request's device occupancy). `net_dev_xmit` remains **disabled via `#if 0`** (never consumed; cuts net event volume). cgroup attribution per subsystem:
...
| block | `block_rq_insert` → `block_rq_issue` (+ `block_rq_complete`) | request_addr | scheduler-queue: other cgroup issued in between; **device-queue**: my `[insert,complete)` overlaps a foreign request's `[issue,complete)` (I waited behind it at the device) |
| softirq | `softirq_entry` → `softirq_exit` (per CPU) | cpu | duration split by per-cgroup event-count ratio into self/other |

Intervals are stored as `portion` sets so overlaps merge automatically; the union is **clamped to the cgroup's `[first_ts,last_ts]` span** before subtraction (prevents out-of-span softirq_other from driving a negative makespan). CPU and block scheduler-queue waits also count the **leading slice** `[start, next event)` when a neighbor already held the resource at the victim's enqueue/insert. Block additionally adds the **device-queue wait** (`block_lifetimes` matched insert→issue→complete by `request_addr` with FIFO reuse handling; the foreign device-occupancy union intersected with the victim's request lifetime) — this captures device-level waiting that the scheduler-queue model alone misses (raises HDD-contention removal from ~20–34% to ~76–86%). It cannot over-remove: with no foreign request in-flight the device wait is empty. Final result per cgroup:
