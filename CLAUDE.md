...
- `docs/figures-reproduction.md` — 논문 figure↔스크립트↔데이터 매핑 + 전체 재생성(`experiments/make_all_figures.sh`) + 측정 주의사항(accuracy는 pairwise / overhead는 32MB ring buffer 빌드 / input_variance는 `reexp_input_variance`(iv_face lv1 cold-start 제외) / block은 디스크 전제).
...
- **Limit NCQ depth: `echo 2 > /sys/block/<dev>/device/queue_depth`** (restore to 32 after). At the default depth 32, NCQ dispatches many requests to the device at once so block contention hides in `issue→complete` device service time — which PureTime cannot see (`block_rq_complete` is `#if 0`), giving only ~39% removal. depth=2 serializes contention into the OS scheduler queue (`insert→issue`), which PureTime *does* measure → block removal rises to **~91% @ 3.2× (K=50, `noise_free ≈ solo`, nf/solo 1.20)**. (depth=1 over-serializes and over-removes: `noise_free < solo`.) This is a measurement prerequisite analogous to disabling NIC offload; `exp_accuracy_by_type.sh` sets/restores it via `BLOCK_QUEUE_DEPTH` (default 2). depth=2 also makes an **I/O-bound block victim viable** (no saturation → no seek dilation), which is why the block victim runs the compression image in **store mode** (`COMPRESS_METHOD=stored` = zip without compression = archiving; minimal CPU → bigger inflation 3.2× and higher removal than DEFLATE's 2.5×/87%).
- **⚠️ Block also requires a realistically-filled (non-empty) HDD** — a second prerequisite as critical as queue_depth=2 (verified 2026-06-17). On an **empty/fresh disk** the victim/fio I/O lands on contiguous fast tracks (solo ~2090ms) so contention never piles up in the `insert→issue` queue → removal collapses to **~45%** even with queue_depth=2. On a **filled/fragmented disk** (as a real serverless node's disk is — other tenants' data) the I/O is slower (solo ~2940ms) so contention queues and is captured → **~91%**. Both regimes are internally stable (std ~7-8 over K=50/20), so the 45↔91 swing is *disk state*, not measurement noise. The committed `accuracy_K50` (91%) was measured on a filled disk and is the valid/realistic value; do NOT "clean up" `/mnt/hdd` before a block run (deleting bulk files empties the disk → 45%). A persistent fill is the cheap fix; an explicit fio pre-fill is too slow on HDD (120GB random-write ≫ 1h) to use as a per-run step.
...
## Other directories
- `funcs/` — victim workloads (float, compression, graph-bfs, network-uploader, thumbnailer, video-processing, ctxsw-bench). **CPU victim/stressor = `float`** (register/L1-bound sqrt/sin/cos 루프 — graph-bfs는 메모리 바운드라 IPC dilation 누수 → 정확도 CPU 실험엔 부적합). **오버헤드 지연 victim(exp5/C7) = `ctxsw-bench`**: 부모-자식 pipe 핑퐁으로 sched_switch를 결정적 생성(같은 코어 협력 실행 → CPU 경쟁 노이즈 없이 이벤트율만 제어). PureTime 오버헤드는 이벤트 수에 비례하므로 `COMPUTE_PER_ROUND`로 이벤트율을 sweep해 "이벤트율 vs 오버헤드" 곡선(전 구간 양수)을 그린다 — 단순 w/vs w/o(오버헤드<1%가 측정 노이즈에 묻혀 절반 음수)의 "더 빠름" 논란을 근본 해소. (자세한 건 [[puretime-experiment-campaign]] + contract C7.) **Block victim = `compression` (store 모드, `COMPRESS_METHOD=stored` = zip 무압축 = archiving, I/O-bound)**. queue_depth=2가 경합을 큐잉 층으로 직렬화해 디스크 포화/seek dilation을 막으므로 I/O-bound victim이 가능 → DEFLATE(CPU-heavy, 2.5×/87%)보다 inflation·removal 모두 우수(3.2×/92%). 순수 `dd`는 store와 달리 측정 전제(queue_depth=2) 없이는 포화→seek dilation. (`udp-sender`는 UDP라 PureTime TCP-TX 범위 밖 → 삭제됨.) **interval-merge victim(exp2/C3) = `video-processing`** (numpy+boto3): 단일스레드 파이프라인이 아니라 **동시 2-스레드**(`cpu_worker` grayscale + `net_worker` MinIO sustained 업로드)에 **스레드별 CPU affinity**(`os.sched_setaffinity`: cpu→경합 코어, net→빈 `IO_CORE`)를 줘 한 cgroup의 CPU wait ∩ Net wait이 **시간상 겹치게** 만든다. 단일스레드는 자원을 직렬화해 겹침≈0, Block은 service-time이 범위 밖이라 제외(자세한 건 [[puretime-experiment-campaign]] + `docs/claims-experiments-contract.md` 1-3).
- `experiments/` — experiment runners (`exp_accuracy_by_type.sh`, `exp_overhead_{time,resource}.sh`, `exp_overhead_ctxsw.sh`=time-overhead vs event-rate/C7, `exp_mixed_noise.sh`=interval-merge/C3), figure generation (`plot_evaluation.py`, `plot_overhead_ctxsw.py`=fig3, `plot_exp2_interval_merge.py`=fig7, `plot_exp3_input_variance.py`, `plot_exp4_baseline.py`; **전체 재생성 = `make_all_figures.sh`, figure↔스크립트↔데이터 매핑·측정 주의는 `docs/figures-reproduction.md`**), and its in-sync copy of `noise_free_makespan.py` (see the "two analyzer copies" note above — change both).
